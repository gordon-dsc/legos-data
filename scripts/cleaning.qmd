...
Since we needed to rectangle the themes and sets data before joining them, these code cells were placed in the cleaning.qmd. However, the lego_data.csv was written to the imported_data folder since it was technically our imported API data.

```{r csv}

# write lego_data csv file to imported_data folder
write_csv(lego_data,
          file = "../data/imported_data/lego_data.csv")
```
